...
public class FrictionFlowRainController : MonoBehaviour
{
	...
	public class FrictionFlowRainDrawerContainer : RainDrawerContainer<DropTrail>
    {
        ...
        {
            get { return Drawer.material != null && Drawer.able == true; }
        }
    ...
    private void InitializeDrawer(FrictionFlowRainDrawerContainer dc)
    {
        ...
        dc.transform.localPosition = RainDropTools.GetSpawnLocalPos(this.transform, _camera, 0f, Variables.SpawnOffsetY);
    ...
    private Vector3 GetNextPositionWithFriction(FrictionFlowRainDrawerContainer dc, float downValue, int resolution, int widthResolution, float dt)
    {
        ...
        Vector3 downward = RainDropTools.GetGForcedScreenMovement(this._camera.transform, this.GForceVector);
        ...
        for (int i = 0; i < iter; i++)
        {
            ...
            for (int j = 0; j <= resol; j++)
            {
                ...
                Vector3 downVector2viewPoint = this._camera.WorldToViewportPoint(downPos);
    ...
    private void UpdateTransform(FrictionFlowRainDrawerContainer dc)
    {
        ...
        dc.Drawer.vertexDistance = (1f * this.Distance * RainDropTools.GetCameraOrthographicSize(this._camera).y) / (Variables.Resolution * 10f);
